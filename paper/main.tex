...
descendants all build their kernel bases out of the irreducible representations of a
compact group acting unitarily, and the surrounding generalisation theory inherits
the same assumption.
...
\paragraph{Equivariant architectures.} Group-equivariant CNNs, steerable CNNs,
tensor-field networks and $\SO(3)$/$SE(3)$ architectures construct layers from
intertwiners of unitary representations. Vector-neuron and geometric-vector-perceptron
designs realise $\ell{=}1$ features with inner and cross products, which is the
primitive set we adopt --- but all of these presuppose an orthogonal action on the
...
\paragraph{Generalisation benefit of symmetry.} A line of work quantifies the strict
generalisation benefit of invariant models via the Reynolds operator, typically for
orthogonal actions and kernel/linear classes. Section~\ref{sec:gen} revisits exactly
which step of that argument uses orthogonality.

\paragraph{ECG and the vectorcardiogram.} Deriving a VCG through the inverse
Dower or Kors transform is standard practice, and \emph{rotation augmentation} in VCG
space is established --- notably in contrastive pre-training (3KG) and in recent work
learning representations in VCG space. These treat the rotation as a data
...
\item[(b)] \emph{(Failure in the estimator's geometry.)} Let $k(x,y) = \phi(\langle
  x,y\rangle)$ be a dot-product kernel with $\phi$ non-degenerate. Then $k$ is
  $G$-invariant iff $\rhoext$ acts orthogonally. For $G_D$ with $\kappa(D) > 1$ the
  standard kernels are \emph{not} $G_D$-invariant, $\mathcal{Q}$ acting on the RKHS is
  an \emph{oblique} projection, and the Pythagorean identity $\|f\|^2 = \|\mathcal{Q}
  f\|^2 + \|f - \mathcal{Q}f\|^2$ --- which every norm-based excess-risk bound uses ---
  fails.
...
The ring of $\Ortho(3)$-invariants of $m$ vectors is generated by the Gram matrix
...
\item[(a)] If $\rank V = 3$, the Gram matrix together with one non-vanishing triple
  ...
  pose is exactly recoverable. The Gram matrix is thus a \emph{complete and lossless}
...
channels. Five primitives, each provably commuting with the action, then build the
...
cross products $v_a \times v_b$; and a gated nonlinearity that rescales vectors by a
scalar gate. The invariant head pools through the trajectory Gram matrix of
Theorem~\ref{thm:ident}; the pose head emits two equivariant vectors that are
...
The public corpora named in our protocol (PTB-XL, CODE-15\%, MIMIC-IV-ECG) are hosted
on PhysioNet and Zenodo, which were not reachable from the execution environment used
...
Gaussian-mixture ECG model: each of the P, QRS and T complexes is a sum of Gaussian
...
\begin{table}[t]\centering
\caption{H1: test macro-F1 by training-set size.}
\label{tab:h1}
\input{tables/h1}
\end{table}
